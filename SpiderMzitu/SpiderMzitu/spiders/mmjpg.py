...
class MmjpgSpider(scrapy.Spider):
    ...
    def parse_step_1(self, response):
        if response.status == 200 or response.status == 301:
            max_num = response.css("#page > a:nth-last-child(3)::text").extract_first(default="1")
            type = u'妹子图'
            title = response.css(".main > .article > h2::text").extract_first(default='NAN')
            img_path = response.css("#content > a > img::attr(src)").extract_first(default='')
